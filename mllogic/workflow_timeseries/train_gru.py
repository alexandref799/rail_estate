...
def parse_args():
    p = argparse.ArgumentParser(description="Train GRU model for gare pricing.")
...
def main():
    ...
    model = build_gru(input_shape=data["X_train_seq"].shape[1:], cfg=cfg)
    ...
    model_path = cfg.models_dir / f"gru_{args.gare}.keras"
    model.save(model_path)

    metrics_path = cfg.outputs_dir / f"metrics_gru_{args.gare}.json"
    ...
    comparison_path = cfg.outputs_dir / f"comparison_gru_{args.gare}.csv"
