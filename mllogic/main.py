...
from mllogic.model_xgbregressor import search_xgbregressor
from mllogic.model_randomforest import search_model_randomforestregressor
from mllogic.model_gradientboosting import search_gbregressor
...
def train(model_name, all_gares):
    ...
    df_taux = _load_single_csv(bucket_name='rail-estate-data', uri_file='taux/taux_mom.csv') #déjà clean
    ...
    if all_gares == 0:
        ...
        df_merge_with_new_gare = run_feature_engineering(df_dvf_clean, df_gare_clean, df_ban_clean, df_taux)
    ...
    if model_name == 'ml-xgb':
        ...
        best_model, results_model, best_params, y_pred = search_gbregressor(X_train_encoded, X_test_encoded, y_train, y_test, n_estimators=[500], learning_rate=[0.05], max_depth=[8], subsample=[0.7], colsample=[0.8], random_state=42, n_iter=50, cv=5)
        
    print best_model, results_model, best_params, y_pred