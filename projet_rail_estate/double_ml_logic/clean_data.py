...
def clean_data_dvf(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean raw data by
    ...
    df["Code ville"] = df["Code ville"].astype(str)

    df_clean = df[cols_to_keep].copy()

    return df_clean

def clean_data_new_gares(df: pd.DataFrame) -> pd.DataFrame:
    """
    Clean raw data by
    ...
    df_new_gares_clean = df_gares[colonnes_finales].copy()

    # Ajouter un id_gare qui ne collisionne pas avec les gares_id des anciennes gares
    df_new_gares_clean = df_new_gares_clean.reset_index(drop=True)
    df_new_gares_clean["id_gare"] = (100000 + df_new_gares_clean.index).astype(int)

    df_new_gares_clean["is_grand_paris"] = 1

    # Harmoniser les dates et extraire l'année de signature
    # df_gares_clean["date_signature"] = pd.to_datetime(df_gares_clean["date_signature"])
    # df_gares_clean["date_ouverture"] = pd.to_datetime(df_gares_clean["date_ouverture"])
    df_new_gares_clean["annee_signature"] = df_new_gares_clean["date_signature"].dt.year

    return df_new_gares_clean

def clean_data_old_gares(df: pd.DataFrame) -> pd.DataFrame:

    # 1️⃣ Supprimer les modes indésirables
    mots_a_supprimer = ["VAL", "TRAM", "TRAMWAY", "TRAIN"]
    pattern = "|".join(mots_a_supprimer)

    df = df[
        ~df["mode"].str.upper().str.contains(pattern, na=False)
    ]

    # 2️⃣ Garder uniquement les gares d'Île-de-France
    df = df[df["idf"] == 1].copy()

    # 3️⃣ Supprimer toutes les colonnes inutiles
    cols_to_drop = [
        "Geo Shape","nom_so_gar","nom_su_gar","id_ref_ZdC","id_ref_ZdA",
        "tertrain","terrer","termetro","tertram","terval","exploitant",
        "principal","x","y","picto ligne","nom_long","nom_ZdC",
        "nom_ZdA","iderflag","indice_lig","mode","idf","iderligc"
    ]

    df = df.drop(columns=cols_to_drop, errors="ignore")

    # 4️⃣ Réordonner les colonnes : mettre gares_id, nom_iv, res_com... et Geo Point à la fin
    ...
    cols_final = [gares, nom] + [c for c in cols if c not in [gares, nom, geo]] + [geo]

    # Réordonner
    df_old_gares_clean = df[cols_final]

    return df_old_gares_clean

def clean_taux(df_taux: pd.DataFrame) -> pd.DataFrame:
    df = df_taux.copy()
    df["date"] = pd.to_datetime(df["date"])
    df["date"] = df["date"].dt.to_period("M").dt.to_timestamp()
    df = df[["date", "taux"]].drop_duplicates("date").sort_values("date")
    return df

def clean_insee(df_insee: pd.DataFrame) -> pd.DataFrame:
    df = df_insee.copy()
    df["Code ville"] = df["Code ville"].astype(str)
    df["Ville_norm"] = (
        df["Ville"]
