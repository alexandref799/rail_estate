import pandas as pd
from Utils_TS import group_by,split_train_test,create_sequences,create_sequences_multi_horizon,Config
...
from load_data import _load_single_csv_clean, _load_single_csv



def compute_lstm(mode, nom_gare= "Issy", test_start='2022-01-01',test_end='2025-06-01'):
...
# Assurez-vous que ces noms de colonnes correspondent EXACTEMENT à ceux de votre DataFrame
    ...
    colonnes_categorielles = ['Nature mutation', 'Type local', "is_new_gare"]
    ...
    df_merge_with_all_gare = _load_single_csv_clean(bucket_name='rail-estate-data', uri_file='gares/df_merge_with_all_gare.csv')
    ...
    if mode == "new":
        print("enter only new gare")
        df_merge_with_all_gare = df_merge_with_all_gare[df_merge_with_all_gare["is_new_gare"] == 1]


    elif mode == "old":
        print("enter only old gare")
        df_merge_with_all_gare = df_merge_with_all_gare[df_merge_with_all_gare["is_new_gare"] == 0]
    ...
    #ajouter boucle if pour choisir sur quoi on veut analyser

    print("choosing the type of data")
    ...
    df_train, df_test = split_train_test(df_group,cfg=Config,test_start=test_start,test_end=test_end)
    ...
    early_stop = EarlyStopping(
        ...
        patience=30,               # nb d'epochs sans amélioration avant d'arrêter
    ...
    plot = pd.DataFrame(history.history)[["loss","val_loss"]].plot()
